...
Add-CisControl '1.1.1' "Enforce password history >= 24" '1 Account Policies' SecPol "$SA::PasswordHistorySize" '' 'ge' 24 3 3 `
...
Add-CisControl '1.1.2' "Maximum password age <= 365 and not 0" '1 Account Policies' SecPol "$SA::MaximumPasswordAge" '' 'nzle' 365 3 3 `
...
Add-CisControl '1.1.3' "Minimum password age >= 1" '1 Account Policies' SecPol "$SA::MinimumPasswordAge" '' 'ge' 1 2 2 `
...
Add-CisControl '1.1.4' "Minimum password length >= 14" '1 Account Policies' SecPol "$SA::MinimumPasswordLength" '' 'ge' 14 4 4 `
...
Add-CisControl '1.1.5' "Password must meet complexity = Enabled" '1 Account Policies' SecPol "$SA::PasswordComplexity" '' 'eq' 1 4 4 `
...
Add-CisControl '1.1.7' "Store passwords w/ reversible encryption = Disabled" '1 Account Policies' SecPol "$SA::ClearTextPassword" '' 'eq' 0 5 3 `
...
Add-CisControl '1.2.1' "Account lockout duration >= 15 min" '1 Account Policies' SecPol "$SA::LockoutDuration" '' 'ge' 15 2 3 `
...
Add-CisControl '1.2.2' "Account lockout threshold 1-5 (not 0)" '1 Account Policies' SecPol "$SA::LockoutBadCount" '' 'nzle' 5 4 4 `
...
Add-CisControl '1.2.4' "Reset lockout counter >= 15 min" '1 Account Policies' SecPol "$SA::ResetLockoutCount" '' 'ge' 15 2 3 `
...
Add-CisControl '2.2.x1' "Debug programs (SeDebugPrivilege) = Administrators only" '2 Local Policies' SecPol "$PR::SeDebugPrivilege" '' 'eq' 'S-1-5-32-544' 5 4 `
...
Add-CisControl '2.2.x2' "Access this computer from network restricted" '2 Local Policies' SecPol "$PR::SeNetworkLogonRight" '' 'match' 'S-1-5-32-544' 3 3 `
...
Add-CisControl '2.3.1.1' "Block Microsoft accounts" '2 Local Policies' Reg "$SYSPOL" 'NoConnectedUser' 'eq' 3 2 2 `
...
Add-CisControl '2.3.1.2' "Guest account status = Disabled" '2 Local Policies' SecPol "$SA::EnableGuestAccount" '' 'eq' 0 4 3 `
...
Add-CisControl '2.3.1.4' "Limit local blank-password use to console = Enabled" '2 Local Policies' Reg "$LSA" 'LimitBlankPasswordUse' 'eq' 1 4 3 `
...
Add-CisControl '2.3.2.1' "Audit: force subcategory settings = Enabled" '2 Local Policies' Reg "$LSA" 'SCENoApplyLegacyAuditPolicy' 'eq' 1 3 2 `
...
Add-CisControl '2.3.6.1' "Domain member: encrypt/sign secure channel (always) = Enabled" '2 Local Policies' Reg "HKLM:\SYSTEM\CurrentControlSet\Services\Netlogon\Parameters" 'RequireSignOrSeal' 'eq' 1 3 3 `
...
Add-CisControl '2.3.7.3' "Interactive logon: lock on smart-card removal" '2 Local Policies' Reg 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon' 'ScRemoveOption' 'in' @('1','2') 2 2 `
...
Add-CisControl '2.3.7.4' "Machine inactivity limit <= 900s (not 0)" '2 Local Policies' Reg "$SYSPOL" 'InactivityTimeoutSecs' 'nzle' 900 2 2 `
...
Add-CisControl '2.3.8.1' "Network client: always digitally sign SMB = Enabled" '2 Local Policies' Reg 'HKLM:\SYSTEM\CurrentControlSet\Services\LanmanWorkstation\Parameters' 'RequireSecuritySignature' 'eq' 1 4 4 `
...
Add-CisControl '2.3.9.1' "Network server: always digitally sign SMB = Enabled" '2 Local Policies' Reg 'HKLM:\SYSTEM\CurrentControlSet\Services\LanmanServer\Parameters' 'RequireSecuritySignature' 'eq' 1 4 4 `
...
Add-CisControl '2.3.10.2' "Network access: no anonymous SAM enumeration" '2 Local Policies' Reg "$LSA" 'RestrictAnonymousSAM' 'eq' 1 3 3 `
...
Add-CisControl '2.3.10.3' "Network access: no anonymous SAM+share enumeration" '2 Local Policies' Reg "$LSA" 'RestrictAnonymous' 'eq' 1 3 3 `
...
Add-CisControl '2.3.10.9' "Everyone permissions do NOT apply to anonymous" '2 Local Policies' Reg "$LSA" 'EveryoneIncludesAnonymous' 'eq' 0 3 3 `
...
Add-CisControl '2.3.11.1' "No storage of LAN Manager hash (NoLMHash)" '2 Local Policies' Reg "$LSA" 'NoLmHash' 'eq' 1 5 4 `
...
Add-CisControl '2.3.11.7' "LAN Manager auth level = NTLMv2 only, refuse LM+NTLM" '2 Local Policies' Reg "$LSA" 'LmCompatibilityLevel' 'eq' 5 5 4 `
...
Add-CisControl '2.3.11.9' "Min NTLM SSP session security (clients)" '2 Local Policies' Reg 'HKLM:\SYSTEM\CurrentControlSet\Control\Lsa\MSV1_0' 'NTLMMinClientSec' 'eq' 537395200 3 3 `
...
Add-CisControl '2.3.17.1' "UAC: Admin Approval Mode for built-in Admin = Enabled" '2 Local Policies' Reg "$SYSPOL" 'FilterAdministratorToken' 'eq' 1 3 3 `
...
Add-CisControl '2.3.17.2' "UAC: elevation prompt for admins = Prompt for consent (secure desktop)" '2 Local Policies' Reg "$SYSPOL" 'ConsentPromptBehaviorAdmin' 'in' @('1','2') 3 3 `
...
Add-CisControl '2.3.17.5' "UAC: Run all admins in Admin Approval Mode = Enabled" '2 Local Policies' Reg "$SYSPOL" 'EnableLUA' 'eq' 1 5 4 `
...
Add-CisControl '2.3.17.6' "UAC: elevate prompts on the secure desktop = Enabled" '2 Local Policies' Reg "$SYSPOL" 'PromptOnSecureDesktop' 'eq' 1 3 3 `
...
Add-CisControl '2.3.17.7' "UAC: detect application installations = Enabled" '2 Local Policies' Reg "$SYSPOL" 'EnableInstallerDetection' 'eq' 1 2 2 `
...
foreach ($s in $svc) {
  Add-CisControl $s[0] ("{0} = Disabled" -f $s[1]) '5 System Services' Service "" $s[2] 'in' @('4') 3 2 $s[3] $s[4] $s[5] `
...
foreach ($p in @('Domain','Private','Public')) {
  ...
  Add-CisControl "9.$p.1" "Firewall state ($p profile) = On" '9 Windows Firewall' Reg $fw 'EnableFirewall' 'eq' 1 4 3 `
  ...
  Add-CisControl "9.$p.2" "Inbound connections ($p) = Block (default)" '9 Windows Firewall' Reg $fw 'DefaultInboundAction' 'eq' 1 4 3 `
...
foreach ($a in $aud) {
  Add-CisControl $a[0] ("Audit: {0} = {1}" -f $a[1],$a[2]) '17 Advanced Audit Policy' AuditPol "" $a[1] 'auditmatch' $a[2] 2 3 `
    $a[3] 'N/A - detection / visibility gap' $a[4] `
    ("Advanced Audit Policy Configuration > locate `"{0}`" and set = {1}." -f $a[1],$a[2])
}

# ---- 18. Administrative Templates (Computer) ----
Add-CisControl '18.4.1' "Disable SMBv1 client driver (mrxsmb10 Start=4)" '18 Admin Templates' Reg 'HKLM:\SYSTEM\CurrentControlSet\Services\mrxsmb10' 'Start' 'eq' 4 5 4 `
...
Add-CisControl '18.4.2' "Disable SMBv1 server = Enabled" '18 Admin Templates' Reg 'HKLM:\SYSTEM\CurrentControlSet\Services\LanmanServer\Parameters' 'SMB1' 'eq' 0 5 4 `
...
Add-CisControl '18.4.3' "Enable SEHOP = Enabled (DisableExceptionChainValidation=0)" '18 Admin Templates' Reg 'HKLM:\SYSTEM\CurrentControlSet\Control\Session Manager\kernel' 'DisableExceptionChainValidation' 'eq' 0 3 2 `
...
Add-CisControl '18.4.5' "WDigest: do not cache plaintext creds (UseLogonCredential=0)" '18 Admin Templates' Reg 'HKLM:\SYSTEM\CurrentControlSet\Control\SecurityProviders\WDigest' 'UseLogonCredential' 'in' @('0') 5 5 `
...
Add-CisControl '18.5.4.1' "Turn off multicast name resolution (LLMNR) = Enabled" '18 Admin Templates' Reg 'HKLM:\SOFTWARE\Policies\Microsoft\Windows NT\DNSClient' 'EnableMulticast' 'eq' 0 4 5 `
...
Add-CisControl '18.6.x' "Disable NetBIOS/NBT-NS broadcast (NodeType = P-node 2)" '18 Admin Templates' Reg 'HKLM:\SYSTEM\CurrentControlSet\Services\NetBT\Parameters' 'NodeType' 'eq' 2 4 5 `
...
Add-CisControl '18.8.x1' "Disallow autoplay for all drives (NoDriveTypeAutoRun=255)" '18 Admin Templates' Reg 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\Explorer' 'NoDriveTypeAutoRun' 'eq' 255 3 3 `
...
Add-CisControl '18.8.x2' "Disable Autorun commands (NoAutorun=1)" '18 Admin Templates' Reg 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\Explorer' 'NoAutorun' 'eq' 1 3 3 `
...
Add-CisControl '18.9.WI1' "Windows Installer: AlwaysInstallElevated (HKLM) = Disabled" '18 Admin Templates' Reg 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\Installer' 'AlwaysInstallElevated' 'in' @('0') 5 4 `
...
Add-CisControl '18.9.RDP1' "Remote Desktop: always prompt for password (fPromptForPassword=1)" '18 Admin Templates' Reg 'HKLM:\SOFTWARE\Policies\Microsoft\Windows NT\Terminal Services' 'fPromptForPassword' 'eq' 1 3 3 `
...
Add-CisControl '18.9.RDP2' "Remote Desktop: high encryption level (MinEncryptionLevel=3)" '18 Admin Templates' Reg 'HKLM:\SOFTWARE\Policies\Microsoft\Windows NT\Terminal Services' 'MinEncryptionLevel' 'eq' 3 3 3 `
...
Add-CisControl '18.9.RDP3' "Remote Desktop: do not allow password saving (DisablePasswordSaving=1)" '18 Admin Templates' Reg 'HKLM:\SOFTWARE\Policies\Microsoft\Windows NT\Terminal Services' 'DisablePasswordSaving' 'eq' 1 2 3 `
...
Add-CisControl '18.9.LSA1' "LSA protection (RunAsPPL) = Enabled" '18 Admin Templates' Reg 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\System' 'RunAsPPL' 'in' @('1','2') 5 4 `
...
Add-CisControl '18.9.CG1' "Virtualization Based Security = Enabled" '18 Admin Templates' Reg 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\DeviceGuard' 'EnableVirtualizationBasedSecurity' 'eq' 1 4 3 `
...
Add-CisControl '18.9.CG2' "Credential Guard = Enabled (LsaCfgFlags)" '18 Admin Templates' Reg 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\DeviceGuard' 'LsaCfgFlags' 'in' @('1','2') 5 3 `
...
Add-CisControl '18.10.D1' "Microsoft Defender Antivirus enabled (DisableAntiSpyware!=1)" '18 Admin Templates' Reg 'HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender' 'DisableAntiSpyware' 'in' @('0') 5 4 `
...
Add-CisControl '18.10.D2' "Defender real-time protection ON (DisableRealtimeMonitoring!=1)" '18 Admin Templates' Reg 'HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender\Real-Time Protection' 'DisableRealtimeMonitoring' 'in' @('0') 5 4 `
...
Add-CisControl '18.10.D3' "Defender PUA protection = Enabled (PUAProtection=1)" '18 Admin Templates' Reg 'HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender' 'PUAProtection' 'eq' 1 3 3 `
...
Add-CisControl '18.10.SS1' "SmartScreen for Explorer = Enabled" '18 Admin Templates' Reg 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\System' 'EnableSmartScreen' 'eq' 1 3 4 `
...
Add-CisControl '18.10.PS1' "PowerShell Script Block Logging = Enabled" '18 Admin Templates' Reg 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\PowerShell\ScriptBlockLogging' 'EnableScriptBlockLogging' 'eq' 1 3 4 `
...
Add-CisControl '18.10.PS2' "PowerShell Module Logging = Enabled" '18 Admin Templates' Reg 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\PowerShell\ModuleLogging' 'EnableModuleLogging' 'eq' 1 2 3 `
...
Add-CisControl '18.10.WRM1' "WinRM client: disallow Basic auth (AllowBasic=0)" '18 Admin Templates' Reg 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\WinRM\Client' 'AllowBasic' 'eq' 0 3 3 `
...
Add-CisControl '18.10.WRM2' "WinRM service: disallow Basic auth (AllowBasic=0)" '18 Admin Templates' Reg 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\WinRM\Service' 'AllowBasic' 'eq' 0 3 3 `
...
Add-CisControl '18.10.WRM3' "WinRM service: disallow unencrypted traffic (AllowUnencryptedTraffic=0)" '18 Admin Templates' Reg 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\WinRM\Service' 'AllowUnencryptedTraffic' 'eq' 0 3 3 `
